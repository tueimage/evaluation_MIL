...
def load_process_xray14(config):
    ...
    xray_df = ld.load_xray(
        skip_processing, processed_labels_path, classication_labels_path,
        image_path, localization_labels_path, results_path, class_name)
    ...
    df_bbox_test, df_class_test, df_bbox_train = ld.get_train_test(
                                                     xray_df,
                                                     random_state = 1,
                                                     do_stats     = False,
...
def load_preprocess_mura(config):
    ...
    df_train_val, test_df_all_classes = ldm.load_mura(
                                            skip_processing,
                                            processed_train_labels_path,
                                            processed_test_labels_path,
                                            mura_train_img_path,
                                            mura_train_labels_path,
                                            mura_test_labels_path,
                                            mura_test_img_path)

    df_train_final, df_val_final, df_test_final = ldm.prepare_mura_set(
                                                      df_train_val,
                                                      test_df_all_classes,
                                                      class_name)
