...
def filter_dataset(df, positive_obs, negative_obs):

    negative_findings = df.loc[df['Finding Labels'].str.contains(negative_obs)][:10000]
    positive_findings = df.loc[df['Finding Labels'].str.contains(positive_obs)]

    df_filtered = pd.concat([negative_findings, positive_findings])
...
def import_dataset(config):
    ...
    if skip_processing:
        print('Skipping processing. Loading data...')
    ...
    # Processing: create a new database ---------------------------------------
        ...
        if dataset == 'xray':
            ...
            print('  Categorizing entry labels')
            df_labels_class = ld.get_classification_labels(path_labels, False)

            # Match the database to the dataset images.
            print('  Matching the database to the dataset images')
            df_labels_adj = ld.preprocess_labels(df_labels_class, path_image)

            # Filter database: all with category class_name, 10000x 'No Finding"
            print(f'  Filtering database:')
            df_labels_pos = filter_dataset(df_labels_adj, class_name, 'No Finding')
            print(df_labels_pos[class_name].value_counts())

            # # Filter database: patients with >= 1 postitive image for the class category
            # print(f'  Filtering patients with a postitive image for "{class_name}"')
